...
void onMessageArrived(const char* topic,
...
void publishTimerHandler(le_timer_Ref_t timer) {
  ...
  const le_result_t publishResult =
      mqtt_Publish(mqttSession, publishTopic, payload, payloadLen,
                   MQTT_QOS0_TRANSMIT_ONCE, retain);
  LE_INFO("Message published with data=%d.  result=%s", messageData,
...
COMPONENT_INIT {
  ...
  mqtt_SetConnectOptions(
      mqttSession, keepAliveInSeconds, cleanSession, username, mqttPassword,
      NUM_ARRAY_MEMBERS(mqttPassword), connectTimeout, retryInterval);
