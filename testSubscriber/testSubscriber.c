...
void onMessageArrived(const char* topic,
  ...
  LE_INFO("Received message! topic: \"%s\", payload: \"%s\"", topic,
          payloadStr);
}
...
COMPONENT_INIT {
  ...
  LE_FATAL_IF(mqtt_Subscribe(mqttSession, subscribeTopic,
                             MQTT_QOS0_TRANSMIT_ONCE) != LE_OK,
              "failed to subscribe to %s", subscribeTopic);
