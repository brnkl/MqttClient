...
static int mqtt_MessageArrivedHandler(void* context,
                                      char* topicName,
                                      int topicLen,
...
//--------------------------------------------------------------------------------------------------
/**
 * Event id that is used to signal that a message has been received from the
 * MQTT broker.  Events
 * must be used because paho spawns a thread for receiving messages and that
 * means that the
...
//--------------------------------------------------------------------------------------------------
/**
 * Event id for connection lost events from paho.  The justification for this
...
le_result_t mqtt_CreateSession(
    const char* brokerURI,  ///< [IN] The URI of the MQTT broker to connect to.
                            ///Should be in
    ///  the form protocol://host:port. eg. tcp://1.2.3.4:1883 or
    ///  ssl://example.com:8883
    const char*
        clientId,  ///< [IN] Any unique string.  If a client connects to an MQTT
    ///  broker using the same clientId as an existing session, then
    ///  the existing session will be terminated.
    mqtt_SessionRef_t*
        session  ///< [OUT] The created session if the return result is LE_OK
    ) {
  mqtt_Session* s = calloc(sizeof(mqtt_Session), 1);
  LE_ASSERT(s);
  const MQTTClient_connectOptions initConnOpts =
  ...
  const int createResult = MQTTClient_create(&(s->client), brokerURI, clientId,
                                             MQTTCLIENT_PERSISTENCE_NONE, NULL);
  if (createResult != MQTTCLIENT_SUCCESS) {
    LE_ERROR("Couldn't create MQTT session.  Paho error code: %d",
             createResult);
    free(s);
    return LE_FAULT;
  }
  ...
  *session = le_ref_CreateRef(SessionRefMap, s);
  ...
  LE_ASSERT(MQTTClient_setCallbacks(
                s->client, *session, &mqtt_ConnectionLostHandler,
                &mqtt_MessageArrivedHandler, NULL) == MQTTCLIENT_SUCCESS);
 ...
 *
 * @note
 *      All MQTT sessions associated with the client session will be destroyed
 * automatically when
 *      the client disconnects from the MQTT service.
 */
//--------------------------------------------------------------------------------------------------
void mqtt_DestroySession(mqtt_SessionRef_t session  ///< Session to destroy
                         ) {
...
static void mqtt_DestroySessionInternal(mqtt_Session* session) {
  ...
  // It is necessary to cast to char* from const char* in order to free the
  // memory
...
//--------------------------------------------------------------------------------------------------
/**
 * Set the connections options which will be used during subsequent calls to
 * mqtt_Connect().
 */
//--------------------------------------------------------------------------------------------------
void mqtt_SetConnectOptions(
    mqtt_SessionRef_t session,   ///< [IN] Session to set connection options in
    uint16_t keepAliveInterval,  ///< [IN] How often to send an MQTT PINGREQ
                                 ///packet if no other
                                 ///  packets are received
    bool cleanSession,  ///< [IN] When false, restore the previous state on a
                        ///reconnect
    // TODO: will message
  ...
  if (password != NULL) {
    // paho uses null terminated strings for passwords, so the password may not
    // contain any
    // embedded null characters.
    for (size_t i = 0; i < passwordLength; i++) {
      if (password[i] == 0) {
        LE_KILL_CLIENT(
            "Password contains embedded null characters and this is not "
            "currently "
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mqtt_Connect(mqtt_SessionRef_t session) {
  mqtt_Session* s = le_ref_Lookup(SessionRefMap, session);
  if (s == NULL) {
    LE_KILL_CLIENT("Session doesn't exist");
    return LE_FAULT;
 ...
 *
 * @note
 *      TODO: If the connection is lost right as disconnect is called, I think
 * that this function
 *      will return LE_FAULT and the client will not know why.
 */
//--------------------------------------------------------------------------------------------------
le_result_t mqtt_Disconnect(mqtt_SessionRef_t session) {
  mqtt_Session* s = le_ref_Lookup(SessionRefMap, session);
  if (s == NULL) {
    LE_KILL_CLIENT("Session doesn't exist");
    return LE_FAULT;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mqtt_Publish(mqtt_SessionRef_t session,
                         const char* topic,
  ...
  const int publishResult =
      MQTTClient_publish(s->client, topic, payloadLen, (void*)payload,
                         mqtt_QosEnumToValue(qos), retain, dtNotUsed);
...
//--------------------------------------------------------------------------------------------------
/**
 * Subscribe to the given topic pattern.  Topics look like UNIX filesystem
 * paths.  Eg.
 * "/bedroom/sensors/motion".  Patterns may include special wildcard characters
 * "+" and "#" to match
 * one or multiple levels of a topic.  For example. "/#/motion" will match
 * topics
 * "/bedroom/sensors/motion" and "/car/data/motion", but not
 * "/bedroom/sensors/motion/enabled".
 *
 * @return
 *      LE_OK on success or LE_FAULT on failure
 */
//--------------------------------------------------------------------------------------------------
le_result_t mqtt_Subscribe(mqtt_SessionRef_t session,
  ...
  const int subscribeResult =
      MQTTClient_subscribe(s->client, topicPattern, qos);
  le_result_t result = LE_OK;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mqtt_Unsubscribe(mqtt_SessionRef_t session,
                             const char* topicPattern) {
  mqtt_Session* s = le_ref_Lookup(SessionRefMap, session);
  if (s == NULL) {
    LE_KILL_CLIENT("Session doesn't exist");
  ...
  const int unsubscribeResult =
      MQTTClient_unsubscribe(session->client, topicPattern);
  le_result_t result = LE_OK;
 ...
 */
//--------------------------------------------------------------------------------------------------
void mqtt_RemoveConnectionLostHandler(mqtt_ConnectionLostHandlerRef_t handler) {
...
mqtt_MessageArrivedHandlerRef_t mqtt_AddMessageArrivedHandler(
  ...
  if (s->messageArrivedHandler != NULL) {
    LE_KILL_CLIENT(
        "You may only register one message arrived handler per session");
    return NULL;
  } else {
 ...
 */
//--------------------------------------------------------------------------------------------------
void mqtt_RemoveMessageArrivedHandler(mqtt_MessageArrivedHandlerRef_t handler) {
  mqtt_Session* s = le_ref_Lookup(SessionRefMap, handler);
  if (s == NULL) {
    LE_KILL_CLIENT("Session doesn't exist");
    return;
...
//--------------------------------------------------------------------------------------------------
/**
 * Gets the correct quality of service integer value as defined by the MQTT
...
//--------------------------------------------------------------------------------------------------
/**
 * This is the connection lost callback function that is supplied to the paho
 * library.  The
 * function generates an event rather than calling the client supplied callback
 * because this
 * function will be called on a non-Legato thread.
 */
//--------------------------------------------------------------------------------------------------
static void mqtt_ConnectionLostHandler(
    void* context,  ///< context parameter contains the session for which the
                    ///connection was lost
...
//--------------------------------------------------------------------------------------------------
/**
 * The event handler for the connection lost event that is generated by
...
static void connectionLostEventHandler(void* report) {
  ...
  if (s->connectionLostHandler != NULL) {
    ...
    LE_WARN(
        "Connection was lost, but no handler is registered to receive the "
        "notification");
  }
}
...
//--------------------------------------------------------------------------------------------------
/**
 * This is the message arrived callback function that is supplied to the paho
 * library.  The
 * function generates an event rather than calling the client supplied callback
 * because this
 * function will be called on a non-Legato thread.
...
static int mqtt_MessageArrivedHandler(void* context,
  ...
  LE_INFO("MessageArrivedHandler called for topic=%s. Storing session=0x%p",
          topicName, context);
  storedMsg->session = context;

  // When topicLen is 0 it means that the topic contains embedded nulls and
  // can't be treated as a
  // normal C string
...
//--------------------------------------------------------------------------------------------------
/**
 * The event handler for the message arrived event that is generated by
...
static void messageReceivedEventHandler(void* report) {
  ...
  if (s->messageArrivedHandler != NULL) {
    if (storedMsg->topicLength <= MQTT_MAX_TOPIC_LENGTH &&
      ...
      s->messageArrivedHandler(storedMsg->topic, storedMsg->payload,
                               storedMsg->payloadLength,
                               s->messageArrivedHandlerContext);
    } else {
      LE_WARN(
          "Message arrived from broker, but it is too large to deliver using "
          "Legato IPC - "
          "topicLength=%zu, payloadLength=%zu",
          storedMsg->topicLength, storedMsg->payloadLength);
    }
  } else {
    LE_WARN(
        "Message has arrived, but no handler is registered to receive the "
        "notification");
  }
...
static void DestroyAllOwnedSessions(le_msg_SessionRef_t sessionRef,
                                    void* context) {
...
COMPONENT_INIT {
  ...
  receiveThreadEventId = le_event_CreateId("MqttClient receive notification",
                                           sizeof(mqtt_Message*));
  le_event_AddHandler("MqttClient receive notification", receiveThreadEventId,
                      messageReceivedEventHandler);

  connectionLostThreadEventId = le_event_CreateId(
      "MqttClient connection lost notification", sizeof(mqtt_Message*));
  le_event_AddHandler("MqttClient connection lost notification",
                      connectionLostThreadEventId, connectionLostEventHandler);

  le_msg_AddServiceCloseHandler(mqtt_GetServiceRef(), DestroyAllOwnedSessions,
                                NULL);
